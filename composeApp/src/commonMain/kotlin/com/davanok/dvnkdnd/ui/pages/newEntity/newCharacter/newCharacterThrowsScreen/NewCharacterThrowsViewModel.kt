...
class NewCharacterThrowsViewModel(
    ...
    fun loadCharacter() {
        ...
        modifierIdToGroupId = modifierGroups
            .fastFlatMap { g -> g.modifiers.fastMap { it.id to g.id } }
            .toMap()
        modifierIdToModifier = modifierGroups
            .fastFlatMap { g -> g.modifiers.fastMap { it.id to it } }
            .toMap()


        val characterAttributes = character.characterAttributes
        _uiState.update { state ->
            state.copy(
                ...
                savingThrows = getModifiersInfo(DnDModifierTargetType.SAVING_THROW) { characterAttributes[it] },
                skills = getModifiersInfo(DnDModifierTargetType.SKILL) { characterAttributes[it.attribute] }
            )
        }
    }
    ...
    private fun toggleSelectionFor(modifierId: Uuid, target: DnDModifierTargetType) {
        val removed = selectedModifiers.remove(modifierId)
        if (removed) {
            updateUiFor(target)
            return
        }
    ...
    fun selectSavingThrow(modifierId: Uuid) =
        toggleSelectionFor(modifierId, DnDModifierTargetType.SAVING_THROW)
    ...
    private fun updateUiFor(target: DnDModifierTargetType) {
        val characterAttributes = uiState.value.attributes
        _uiState.update {
            when (target) {
                DnDModifierTargetType.SAVING_THROW -> it.copy(
                    savingThrows = getModifiersInfo(
                        DnDModifierTargetType.SAVING_THROW
                    ) { attribute -> characterAttributes[attribute] }
                )

                DnDModifierTargetType.SKILL -> it.copy(
                    skills = getModifiersInfo(
                        DnDModifierTargetType.SKILL
                    ) { skill -> characterAttributes[skill.attribute] }
                )
    ...
    private inline fun <reified E : Enum<E>> getModifiersInfo(
        ...
        val modifiersRaw = mutableMapOf<String, MutableList<ModifierExtendedInfo>>()
        ...
        modifierGroups.fastForEach { group ->
            ...
            group.modifiers.fastForEach { modifier ->
                ...
                val selectable =
                    modifier.selectable && (selected || groupAllowsExtra) // TODO(clampMin/clampMax check)

                val ext = buildExtInfo(modifier, group, selectable, selected)
                modifiersRaw.getOrPut(modifier.target) { ArrayList() }.add(ext)
            }
        }

        val modifiers = mutableMapOf<E, List<ModifierExtendedInfo>>()
        modifiersRaw.forEach { (k, v) ->
            runCatching { enumValueOf<E>(k) }.getOrNull()?.let { modifiers[it] = v }
        }

        val result = modifiers.mapValues { (k, v) ->

            v to calculateModifierSum(
                baseValue = baseValueGetter(k),
                groups = modifierGroups.fastFilter { it.target == target },
                modifierFilter = { it.target == k.name },
                valueGetter = { source ->
                    when (source) {
                        DnDModifierValueSource.CONST -> null
                        DnDModifierValueSource.CHARACTER_LEVEL -> uiState.value.characterLevel
                        DnDModifierValueSource.ENTITY_LEVEL -> TODO()
                        DnDModifierValueSource.PROFICIENCY_BONUS -> uiState.value.proficiencyBonus
                    }?.toDouble()
                }
            )
        }
    ...
    fun commit(onSuccess: () -> Unit) = viewModelScope.launch {
        val isSavingThrowsValid = modifierGroups
            .fastFilter { group -> group.target == DnDModifierTargetType.SAVING_THROW }
            .fastAll { group ->
                group.modifiers.count { it.id in selectedModifiers } == group.selectionLimit
            }
        val isSkillsValid = modifierGroups
            .fastFilter { group -> group.target == DnDModifierTargetType.SKILL }
            .fastAll { group ->
                group.modifiers.count { it.id in selectedModifiers } == group.selectionLimit
            }
        ...
        newCharacterViewModel.setCharacterSelectedModifiers(selectedModifiers).onFailure { thr ->
            _uiState.update {
                it.copy(
